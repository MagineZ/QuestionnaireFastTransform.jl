...
def pyquest(data,params):
    ...
    """
    m,n = data.shape
    neig_col = int(np.floor(np.sqrt(n)/5))
    neig_row = int(np.floor(np.sqrt(m)/5))
    ...
    if params.init_aff_type == INIT_AFF_COS_SIM:
        ...
        Urow,Srow, _ = tree_roseland.DMroseland_metric(data,Dim = neig_row, method = 'euclidean')
        coords = Urow * (Srow**(1/2))[np.newaxis, :]
        sq_norms = np.sum(coords**2, axis=1, keepdims=True)
        D_sq = sq_norms + sq_norms.T - 2 * coords @ coords.T
        D_sq = np.maximum(D_sq, 0)
        init_row_aff = np.exp(-D_sq / np.median(D_sq))    
    ...
    for i in range(params.n_iters):
        ...
        if params.col_affinity_type == DUAL_EMD:
            if params.col_weighted == True:
                ...
                Ucol,Scol, _ = tree_roseland.DMroseland_metric(data.T,Dim = neig_col, method = 'emd1',row_tree = dual_row_trees[-1],alpha = params.row_alpha, 
                                                   beta = params.row_beta)
                coords = Ucol * (Scol**(1/2))[np.newaxis, :]
                sq_norms = np.sum(coords**2, axis=1, keepdims=True)
                D_sq = sq_norms + sq_norms.T - 2 * coords @ coords.T
                col_emd = np.maximum(D_sq, 0)
            ...
            col_aff = dual_affinity.partition_dualgeometry_ref(np.expand_dims(data, axis=2), dual_row_trees[-1], params.col_alpha, 1)
        elif params.col_affinity_type == DUAL_COSINE:
            print("cosine distance.")
            col_aff = dual_affinity.partition_dualgeometry_ref(np.expand_dims(data, axis=2), dual_row_trees[-1], params.col_alpha, 0)
        ...
        if params.row_affinity_type == DUAL_EMD:
            if params.row_weighted == True:
                ...
                Urow,Srow, _ = tree_roseland.DMroseland_metric(data,Dim = neig_row, method = 'emd1',row_tree = dual_col_trees[-1],alpha = params.row_alpha, 
                                                   beta = params.row_beta)
                coords = Urow * (Srow**(1/2))[np.newaxis, :]
                sq_norms = np.sum(coords**2, axis=1, keepdims=True)
                D_sq = sq_norms + sq_norms.T - 2 * coords @ coords.T
                row_emd = np.maximum(D_sq, 0)
            ...
            print("correlation distance.")
            row_aff = dual_affinity.partition_dualgeometry_ref(np.expand_dims(data.T, axis=2), dual_col_trees[-1], params.row_alpha, 1)
        elif params.row_affinity_type == DUAL_COSINE:
            print("cosine distance.")
            row_aff = dual_affinity.partition_dualgeometry_ref(np.expand_dims(data.T, axis=2), dual_col_trees[-1], params.row_alpha, 0)
